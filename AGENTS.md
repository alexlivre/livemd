...
- `src/main/index.ts` — window, IPC handlers (`file:open-dialog`, `file:read`, `tab:close`, `shell:reveal`, `clipboard:write-text`, `app:consume-pending`, `app:get-locale`, `app:set-language`, `app:get-version`, `app:open-external`, `app:check-update`), chokidar watchers, single-instance lock, argv/"Abrir com" handling (`extractMarkdownFromArgs`, `deliverOpenPath`).
- `src/preload/index.ts` — exposes `window.mdApi` (contract in `src/shared/api.ts`). `webUtils.getPathForFile` must be used HERE (Electron 32 removed `File.path`; it does not work from the page). Clipboard writes go through the `clipboard:write-text` IPC handler in main — the sandboxed preload cannot import the `clipboard` module.
...
- **CSP** in `src/renderer/index.html` is `default-src 'self'; script-src 'self'` (no inline scripts/event handlers — attach listeners via `addEventListener`) and `img-src 'self' data: file: https:` — remote images are allowed, but the DOMPurify `afterSanitizeAttributes` hook in `markdown.ts` must keep tagging every `http(s)` `<img>` with `referrerpolicy="no-referrer"` so image hosts never see which file is being read. Static strings are applied via `data-i18n` / `data-i18n-title` / `data-i18n-aria` attributes in `applyStaticStrings()`.
- **IPC/webPreferences**: contextIsolation on, nodeIntegration off, sandbox on — preserve. The sandboxed preload only exposes `contextBridge`, `ipcRenderer`, `nativeImage`, `webFrame`, `webUtils` from Electron (no `clipboard`) — privileged operations (clipboard writes, file reads, dialogs) must go through IPC to main. `app:open-external` only allows the whitelisted repo/releases URLs — keep the whitelist tight.
